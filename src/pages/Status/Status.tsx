...
export const Status: React.FC = () => {
    ...
    const { loading, data, error, refetch } = useQuery(GET_STATUS_PAGE, { variables: { offset: currentPage - 1, limit } });
    const { setTitle } = usePageTitle();
    const [deleteStatus] = useMutation(DELETE_STATUS);
    const totalPages = Math.ceil((data?.statusPage.totalCount ?? 0) / limit);
    ...
    useEffect(() => {
        if(data) {
            setStatusList(data.statusPage.items);
        }
