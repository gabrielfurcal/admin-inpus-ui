...
import { GET_ROUTES_PAGE } from "../../graphql/queries";
...
export const RoutesPage: React.FC = () => {
    ...
    const { loading, data, error, refetch } = useQuery(GET_ROUTES_PAGE, { variables: { offset: currentPage - 1, limit } });
    const { setTitle } = usePageTitle();
    const [deleteRoute] = useMutation(DELETE_ROUTE);
    const totalPages = Math.ceil((data?.routesPage.totalCount ?? 0) / limit);
    ...
    useEffect(() => {
        if(data) {
            setRoutes(data.routesPage.items);
        }
